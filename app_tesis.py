        ...
        if not X.empty:
            ...
            * **Barra Gris:** El dato es incierto (podría ser casualidad).
            ...
            try:
                ...
                # Introducción
                st.markdown("#### Análisis de Potencia: Educación vs. Seguridad")
                st.markdown("Para entender la fuerza de estos resultados, imaginemos que la decisión del usuario es subir una **escalera corta de 4 peldaños**:")
                
                # Lista de niveles
                st.markdown("""
                * 1. **Muy Bajo** (Usuario desconectado)
                * 2. **Bajo**
                * 3. **Alto**
                * 4. **Muy Alto** (Usuario convencido)
                """)
                
                st.markdown("El modelo matemático ha encontrado dos velocidades distintas de ascenso:")
                
                st.divider()
                
                # 1. Educación Financiera
                st.markdown(f"##### 1. El Motor Principal: Educación Financiera ({coef_edu:.2f})")
                st.markdown(f"Este es el hallazgo más importante. El coeficiente es **mayor a 1** ({coef_edu:.2f}). Esto genera un Efecto Trampolín:")
                st.markdown(f"""
                * Si usted educa al usuario para que suba **1 solo peldaño** en su conocimiento (ej. pasar de nivel "Bajo" a "Alto")...
                * Su intención de uso salta automáticamente **{coef_edu:.2f} peldaños**.
                * **Interpretación:** El conocimiento empodera. Quien entiende cómo funciona la herramienta, la usa casi de inmediato.
                """)

                # 2. Seguridad Percibida
                st.markdown(f"##### 2. El Factor de Apoyo: Seguridad Percibida ({coef_seg:.2f})")
                st.markdown(f"Aquí el coeficiente es **{coef_seg:.2f}**. Es positivo, pero más lento:")
                st.markdown(f"""
                * Si el usuario mejora su sensación de seguridad en **1 peldaño**...
                * Su intención de uso avanza **{coef_seg:.2f} peldaños**.
                * **Interpretación:** La seguridad es necesaria, pero no suficiente por sí sola. Sentirse seguro ayuda, pero no provoca la adopción inmediata si no hay entendimiento.
                """)
                
                # Conclusión (Usamos st.success para el recuadro verde nativo)
                st.success("""
                **Conclusión:**
                Invertir en seguridad suma puntos de a poco, pero **invertir en educación financiera multiplica los resultados**. 
